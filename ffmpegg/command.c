...
static int opt_vsync(void *optctx, const char *opt, const char *arg) {
    ...
    if (o->run_context_ref->video_sync_method == VSYNC_AUTO) {
        int has_error = 0;
        o->run_context_ref->video_sync_method = parse_number_or_die(o->run_context_ref->trace_id, "vsync", arg, OPT_INT,
                                                                    VSYNC_AUTO, VSYNC_VFR, &has_error);
        if (has_error < 0) {
            return -1;
        }
    }
...
//          "when dumping packets, also dump the payload" },
        ...
        {"vsync", HAS_ARG | OPT_EXPERT, {.func_arg = opt_vsync},
         "video sync method", ""},
        {"frame_drop_threshold", HAS_ARG | OPT_FLOAT | OPT_EXPERT | OPT_RUN_OFFSET,
         {.off= RUN_CTX_OFFSET(frame_drop_threshold)},
         "frame drop threshold", ""},
        {"async", HAS_ARG | OPT_INT | OPT_EXPERT | OPT_RUN_OFFSET, {.off= RUN_CTX_OFFSET(audio_sync_method)},
         "audio sync method", ""},
        {"adrift_threshold", HAS_ARG | OPT_FLOAT | OPT_EXPERT | OPT_RUN_OFFSET,
         {.off= RUN_CTX_OFFSET(audio_drift_threshold)},
...
//                    "set profile", "profile" },
        ...
        {"filter_complex_threads", HAS_ARG | OPT_INT | OPT_RUN_OFFSET,
         {.off = RUN_CTX_OFFSET(filter_complex_nbthreads)},
         "number of threads for -filter_complex"},
        {"lavfi", HAS_ARG | OPT_EXPERT, {.func_arg = opt_filter_complex},
         "create a complex filtergraph", "graph_description"},
        {"filter_complex_script", HAS_ARG | OPT_EXPERT, {.func_arg = opt_filter_complex_script},
         "read complex filtergraph description from a file", "filename"},
        {"auto_conversion_filters", OPT_BOOL | OPT_EXPERT | OPT_RUN_OFFSET,
         {.off = RUN_CTX_OFFSET(auto_conversion_filters)},
...
//                    "ratio of decoding errors (0.0: no errors, 1.0: 100% errors) above which ffmpeg returns an error instead of success.", "maximum error rate" },
        ...
        {"find_stream_info", OPT_BOOL | OPT_PERFILE | OPT_INPUT | OPT_EXPERT | OPT_RUN_OFFSET,
         {.off = RUN_CTX_OFFSET(find_stream_info)},
        ...
        {"bits_per_raw_sample", OPT_VIDEO | OPT_INT | HAS_ARG | OPT_RUN_OFFSET,
         {.off = RUN_CTX_OFFSET(frame_bits_per_raw_sample)},
...
//          "show available HW acceleration methods" },
        ...
        {"isync", OPT_BOOL | OPT_EXPERT, {.off = RUN_CTX_OFFSET(input_sync)},
...
//            exit_program(1);
        ...
        for (i = 0; i < run_context->option_input.input_files[sync_file_idx]->nb_streams; i++)
            if (check_stream_specifier(run_context->option_input.input_files[sync_file_idx]->ctx,
                                       run_context->option_input.input_files[sync_file_idx]->ctx->streams[i], sync) ==
                1) {
                sync_stream_idx = i;
                break;
            }
...
//            exit_program(1);
        ...
        if (run_context->option_input.input_streams[run_context->option_input.input_files[sync_file_idx]->ist_index +
...
//            exit_program(1);
        ...
        if (negative)
            ...
            for (i = 0; i < run_context->option_input.input_files[file_idx]->nb_streams; i++) {
                if (check_stream_specifier(run_context->option_input.input_files[file_idx]->ctx,
                                           run_context->option_input.input_files[file_idx]->ctx->streams[i],
                                           *p == ':' ? p + 1 : p) <= 0)
                    continue;
                if (run_context->option_input.input_streams[run_context->option_input.input_files[file_idx]->ist_index +
...
//        exit_program(1);
    ...
    if (m->channel_idx < 0 || m->channel_idx >= st->codecpar->channels ||
        run_context->option_input.input_streams[run_context->option_input.input_files[m->file_idx]->ist_index +
...
//        exit_program(1);
    ...
    *p++ = '\0';
    ...
    o->streamid_map = grow_array(trace_id, o->streamid_map, sizeof(*o->streamid_map), &o->nb_streamid_map, idx + 1,
                                 &has_error);
    if (has_error < 0) { return -1; }
...
static int opt_filter_complex_script(void *optctx, const char *opt, const char *arg) {
    ...
    if (!(o->run_context_ref->filtergraphs[o->run_context_ref->nb_filtergraphs - 1] = av_mallocz(
            sizeof(*o->run_context_ref->filtergraphs[0]))))
        return AVERROR(ENOMEM);
    o->run_context_ref->filtergraphs[o->run_context_ref->nb_filtergraphs - 1]->index =
            o->run_context_ref->nb_filtergraphs - 1;
...
static int opt_filter_complex(void *optctx, const char *opt, const char *arg) {
    OptionsContext *o = optctx;
    int has_error = 0;
    GROW_ARRAY(o->run_context_ref->trace_id, o->run_context_ref->filtergraphs, o->run_context_ref->nb_filtergraphs,
               has_error);
    if (has_error < 0) {
        return -1;
    }
...
int run_ffmpeg_cmd(char *trace_id, char *cmd) {
    ...
    if (parent_context.raw_context.option_output.nb_output_files <= 0 &&
        parent_context.raw_context.option_input.nb_input_files == 0) {
